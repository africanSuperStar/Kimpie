...
class DefaultAstrocyteSharedDatabase(driver: Single<SqlDriver>) : AstrocyteSharedDatabase {

    constructor(driver: SqlDriver) : this(singleOf(driver))

    private val queries: Single<AstrocyteDatabaseQueries> =
        driver
            .map { AstrocyteDatabase(it).astrocyteDatabaseQueries }
            .asObservable()
            .replay()
            .autoConnect()
            .firstOrError()
    ...
    override fun observeAll(): Observable<List<AstrocyteItemEntity>> =
        query(AstrocyteDatabaseQueries::selectAll)
    ...
    private fun <T : Any> query(query: (AstrocyteDatabaseQueries) -> Query<T>): Single<Query<T>> =
        queries
            .observeOn(ioScheduler)
            .map(query)

    private fun execute(query: (AstrocyteDatabaseQueries) -> Unit): Completable =
        queries
            .observeOn(ioScheduler)
